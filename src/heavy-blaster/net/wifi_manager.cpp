...
void WifiManager::stop() {
  ...
  lastAttemptMs_ = 0;
  warnedMissingConfig_ = false;
  Serial.println("[heavy-blaster][wifi] stopped");
}
...
void WifiManager::loop(const RuntimeConfig& config) {
  ...
  if (config.wifiSsid.length() == 0) {
    if (!warnedMissingConfig_) {
      Serial.println("[heavy-blaster][wifi] missing wifi.ssid; provision config first");
  ...
  lastAttemptMs_ = now;
  Serial.println("[heavy-blaster][wifi] connecting to configured SSID");
