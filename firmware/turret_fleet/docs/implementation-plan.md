...
| Public release pipeline exists conceptually | `.github/workflows/boss-target-firmware.yml:74-128` builds fleet firmware and uploads release assets. `scripts/turret_fleet/make_release_manifest.py:18-50` writes manifest fields. | Keep public GitHub release artifacts, but align manifest identity with rebuilt firmware constants and Command Center rollout policy. |
