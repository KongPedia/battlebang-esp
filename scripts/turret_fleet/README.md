...
- `repeat_lane_sweep_live.py`: repeatedly publishes live `lane_sweep` commands. Add `--boss-id <boss_target_id>` to act like a simple Command Center opening: publish boss `reset`, wait for READY, publish boss `start` to begin the 5-second neon-rainbow orbit intro, immediately publish `home` to each configured turret so home overlaps the intro, then begin turret lane sweeps after both the intro and home commands finish. While running with `--boss-id`, it also watches boss status and publishes `dead` to each turret once boss HP reaches 0.
...
Opening order with `--boss-id` is: boss HP reset -> boss neon intro starts + turret `home` starts immediately -> lane_sweep after both are done. Use `--no-turret-home-first` only when the turrets are already homed and you intentionally want to skip that reset step.
