...
3. **MQTT-ready initial boot**: after boot, the turret must not idle sweep or fire automatically. A configured turret starts Wi-Fi/MQTT automatically and, after MQTT is connected, aims at local `motion.home` (default `yaw=0,pitch=0`) as a no-fire boot HOME target; world `(x,y,z)` target solving is only for explicit target commands.
...
- Boot log starts at `WAIT_COMMAND`, then after MQTT connection reports a no-fire local `HOME`, never `IDLE`.
