...
def build_command_payload(args: argparse.Namespace) -> tuple[str, dict[str, Any]]:
    ...
    if action == "config":
        ...
        if args.fire_relay_profile is not None:
            fire["relay_profile"] = args.fire_relay_profile
...
def build_parser() -> argparse.ArgumentParser:
    ...
    cfg.add_argument(
        "--fire-relay-profile",
        choices=["active_low", "active_high", "two_channel_active_low", "single_channel_ch3_active_high"],
        help="named relay polarity preset saved to NVS",
    )
