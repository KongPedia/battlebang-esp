...
void WifiManager::stop() {
  ...
  lastAttemptMs_ = 0;
  warnedMissingConfig_ = false;
  Serial.println("[boss_target][wifi] stopped");
}
...
void WifiManager::loop(const RuntimeConfig& config) {
  ...
  if (config.wifiSsid.length() == 0) {
    if (!warnedMissingConfig_) {
      Serial.println("[boss_target][wifi] missing wifi.ssid; provision config first");
  ...
  lastAttemptMs_ = now;
  Serial.println("[boss_target][wifi] connecting to configured SSID");
