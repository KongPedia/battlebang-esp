...
void WifiManager::stop() {
  ...
  lastAttemptMs_ = 0;
  warnedMissingConfig_ = false;
  Serial.println("[hit_target][wifi] stopped");
}
...
void WifiManager::loop(const RuntimeConfig& config) {
  ...
  if (config.wifiSsid.length() == 0) {
    if (!warnedMissingConfig_) {
      Serial.println("[hit_target][wifi] missing wifi.ssid; provision config first");
  ...
  lastAttemptMs_ = now;
  Serial.println("[hit_target][wifi] connecting to configured SSID");
