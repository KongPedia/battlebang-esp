...
#include "boss_target/mqtt/mqtt_bus.h"
...
using battlebang::boss_target::MqttBus;
using battlebang::boss_target::OtaManifest;
using battlebang::boss_target::OtaResult;
using battlebang::boss_target::RuntimeConfig;
using battlebang::boss_target::RuntimeConfigStore;
using battlebang::boss_target::WifiManager;
using battlebang::boss_target::applyRuntimeConfigJson;
using battlebang::boss_target::fetchHttpText;
using battlebang::boss_target::gameplayConfigChanged;
using battlebang::boss_target::ledHardwareChanged;
using battlebang::boss_target::makeDefaultRuntimeConfig;
using battlebang::boss_target::parseOtaManifestJson;
using battlebang::boss_target::runtimeConfigToJson;
using battlebang::boss_target::runHttpOta;
using battlebang::boss_target::sensorPinsChanged;
using battlebang::boss_target::shouldApplyOtaManifest;
using battlebang::boss_target::otaManifestSummary;
using battlebang::boss_target::consumeOtaRebootMarker;
using battlebang::boss_target::writeOtaRebootMarker;
...
void printStatusJson(const char* reason,
  ...
  doc["reason"] = reason;
  doc["source"] = source;
  doc["target_index"] = targetIndex == 255 ? -1 : static_cast<int>(targetIndex);
  doc["peak"] = peak;
...
void onTargetEvent(const BossTargetEvent& event, void*) {
  // Keep the per-ring index visible on serial JSON for bench validation.
  // 255 means "not tied to one target" (reset/status/start rejection).
  printStatusJson(event.name, event.name, event.peak, event.source, event.digitalEdges, event.targetIndex);
  publishMqttStatusIfConnected(event.name);
}

void startNetwork(const char* reason) {
  if (networkStarted) return;
  Serial.print("[boss_target][network] starting reason=");
...
void stopNetwork(const char* reason) {
  Serial.print("[boss_target][network] stopping reason=");
  Serial.println(reason);
...
String configuredOtaPollUrl() {
  ...
  return config.otaPublicManifestUrl.length() > 0 ? config.otaPublicManifestUrl : String(BB_BOSS_TARGET_LATEST_MANIFEST_URL);
}
...
void checkOtaManifestUrlWithPolicy(const String& url, bool requireCommandCenterApproval, const char* source) {
  if (!wifi.connected()) {
    Serial.println("[boss_target][ota] Wi-Fi is not connected; cannot check OTA");
  ...
  if (!fetchHttpText(url, 4096, body, error)) {
    Serial.print("[boss_target][ota] manifest fetch failed: ");
  ...
  if (!parseOtaManifestJson(body.c_str(), manifest, error)) {
    Serial.print("[boss_target][ota] manifest parse failed: ");
    Serial.println(error);
  ...
  if (requireCommandCenterApproval) {
    ...
    if (!commandCenterApprovesPolledOta(manifest, approvalReason)) {
      Serial.print("[boss_target][ota] auto poll not approved: ");
      Serial.println(approvalReason);
      publishMqttStatusIfConnected("ota_not_approved");
      return;
    }
    Serial.print("[boss_target][ota] auto poll approved: ");
    Serial.println(approvalReason);
  }
  String reason;
  ...
  Serial.print("[boss_target][ota] accepted from ");
  Serial.print(source);
  Serial.print(' ');
...
void applyAndPersistConfig(const char* json, const char* source) {
  ...
  if (!applyRuntimeConfigJson(json, next, error)) {
    Serial.print("[boss_target][serial] config rejected: ");
    Serial.println(error);
  ...
  if (hardwareChanged) {
    Serial.println("[boss_target][config] LED pin/type/order are hardware-profile values; reboot/rebuild may be required for those fields");
  }
  if (wifiChanged && networkStarted) wifi.begin(config);
  if (mqttStarted && mqttChanged) {
    Serial.println("[boss_target][serial] MQTT identity/settings changed; reconnecting/resubscribing");
    mqtt.reconfigure();
  }
  if (!networkStarted && config.networkAutoStart && millis() >= config.networkStartDelayMs) startNetwork("config_auto_start");
  Serial.print("[boss_target][serial] config applied saved=");
...
void clearConfig() {
  ...
  Serial.print("[boss_target][serial] config cleared=");
...
void handleCommandLine(String line) {
  ...
  if (lower == "start" || lower == "arm" || lower == "activate") {
    target.start("serial", true);
    return;
  }
  ...
  if (lower == "h" || lower == "hit") {
    if (!config.debugAllowSimulateHit) {
      Serial.println("[boss_target][serial] simulate hit rejected; debug_allow_simulate_hit=false");
      printStatusJson("simulate_hit_rejected", "command_rejected", 0, "serial");
      publishMqttStatusIfConnected("command_rejected");
      return;
    }
  ...
  Serial.print("[boss_target][serial] unknown command: ");
  Serial.println(line);
  printHelp();
}
...
void pollConfiguredOta() {
  ...
  if (config.otaCommandCenterControlled &&
      (config.otaDesiredBuild == 0 || config.otaDesiredBuild <= BB_BOSS_TARGET_BUILD)) {
    return;
  }
  ...
  if (url.length() == 0) {
    Serial.println("[boss_target][ota] auto poll skipped: no manifest URL configured");
    publishMqttStatusIfConnected("ota_poll_no_url");
    return;
  }
  Serial.print("[boss_target][ota] auto polling ");
  Serial.println(url);
...
void setup() {
  Serial.begin(::boss_target::SERIAL_BAUD);
  delay(200);
  ...
  Serial.println(BB_BOSS_TARGET_RELEASE_REPO);
  Serial.print("latest_manifest=");
  Serial.println(BB_BOSS_TARGET_LATEST_MANIFEST_URL);
  if (postOtaReboot) Serial.println("[boss_target][ota] post-OTA reboot marker consumed");
  Serial.println(runtimeConfigToJson(config, false));
  printHelp();
  target.reset("boot");
