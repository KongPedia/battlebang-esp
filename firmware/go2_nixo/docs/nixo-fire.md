...
- `relay_1ch` / `esp32dev_go2_nixo` or `esp32dev_go2_nixo_1ch` envs:
  - relay 1: `GPIO23`
  - relay 2: disabled (`-1`)
  - relay polarity: active-HIGH (`HIGH` fires, `LOW` stops)
- `relay_2ch` / `esp32dev_go2_nixo_2ch` env:
...
Variant config lives in `firmware/go2_nixo/variants/<variant>/config.json` and is selected by PlatformIO `custom_nixo_variant` or the `GO2_NIXO_RELAY_VARIANT` / `NIXO_RELAY_VARIANT` shell override. Pins, polarity, and channel count stay build variant data; fire duration/cooldown/prefire/relay-delay timing is NVS runtime tuning.
...
```bash
# Default one-channel integrated fallback
pio run -e esp32dev_go2_nixo_1ch
...
```text
[NIXO] mqtt=enabled nixo_id=nixo_go2_06 command_topic=battlebang/nixo/nixo_go2_06/command relay1=22 relay2=23 relay_on=0 relay_off=1 delay1_ms=150 fire_default_ms=3000 fire_min_ms=100 fire_max_ms=10000 cooldown_ms=1500 prefire_ms=600
