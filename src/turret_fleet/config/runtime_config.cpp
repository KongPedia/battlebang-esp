...
bool applyRuntimeConfigJson(const char* json, RuntimeConfig& config, String& error) {
  ...
  if (!fire.isNull()) {
    ...
    if (fire["relay_active_low"].is<bool>()) {
      const bool activeLow = fire["relay_active_low"].as<bool>();
      next.fireRelayActiveLow = activeLow;
    ...
    if (fire["relay_ch2_active_low"].is<bool>()) {
      next.fireRelayCh2ActiveLow = fire["relay_ch2_active_low"].as<bool>();
    }
    if (fire["relay_ch3_active_low"].is<bool>()) {
      next.fireRelayCh3ActiveLow = fire["relay_ch3_active_low"].as<bool>();
    }
  }
...
String runtimeConfigToJson(const RuntimeConfig& config, bool includeSecrets) {
  ...
  fire["relay_active_low"] = config.fireRelayActiveLow;
...
bool RuntimeConfigStore::save(const RuntimeConfig& config) {
  ...
  ok &= prefs.putBool("fire_rel_al", config.fireRelayActiveLow) > 0;
