...
// - Nixo fire is handled by the separate nIxo ESP through MQTT.
...
void setup() {
  ...
  Serial.println("[MODE] go2 ADC threshold hit candidate + server ring display; D0 is debug-only; Nixo fire is separate ESP");
  Serial.printf("[PIN] LED=%d count=%d | PIEZO_AO=%d | PIEZO_DO_DEBUG=%d | BT=%s\n",
                LED_PIN,
                NUM_LEDS,
                PIEZO_AO_PIN,
                PIEZO_DO_PIN,
