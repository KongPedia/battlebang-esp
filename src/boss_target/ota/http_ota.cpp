...
bool ensureTlsClock() {
  ...
  Serial.println("[boss_target][ota] syncing clock for HTTPS certificate validation");
  ...
  Serial.println("[boss_target][ota] TLS clock sync timed out");
  return false;
}
...
bool fetchHttpText(const String& url, size_t maxBytes, String& body, String& error) {
  ...
  Serial.print("[boss_target][http] GET ");
  Serial.println(url);
...
OtaResult runHttpOta(const OtaManifest& manifest) {
  ...
  Serial.print("[boss_target][ota] downloading ");
  ...
  while (http.connected() && (contentLength < 0 || written < static_cast<size_t>(contentLength))) {
    ...
    if (now - lastProgressLogMs > 2000) {
      ...
      Serial.print("[boss_target][ota] bytes=");
